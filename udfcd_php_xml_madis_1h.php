 ...
 */
  ...
  echo 'Processing began at '.date("F j, Y, g:i a").'  Pulling back 75 minutes.<br />';
    /*
...
//view-source:https://madis-data.noaa.gov/madisPublic/cgi-bin/madisXmlPublicDir?rdr=&time=0&minbck=-60&minfwd=0&recwin=3&dfltrsel=2&state=CO&latll=0.0&lonll=0.0&latur=90.0&lonur=0.0&stanam=&stasel=0&pvdrsel=0&varsel=1&qcsel=1&xml=1&csvmiss=0&nvars=STALOC
     ...
     */
$counter1=0;
    echo '<br />Processing 1 Hour Precip (PCP1H), '.date("F j, Y, g:i a").'<br />';
    ...
    echo '<br />Processing 1 Hour Precip from UDFCD (hydro param), '.date("F j, Y, g:i a").'<br />';
      ...
      if( ! $xml = simplexml_load_string($xmlstring2) ) {
        ...
        echo '   Inserting 1 Hour UDFCD Precip, '.date("F j, Y, g:i a").'<br />';
        $counter2=0;
        foreach( $xml as $response ) {
          ++$counter1;
          ++$counter2;
    ...
    echo '  ...1 Hour UDFCD Precip done! '.$counter2.' records inserted. '.date("F j, Y, g:i a").'<br />';
      }
    echo '<br />Processing Wind (FF, FFGUST), '.date("F j, Y, g:i a").'<br />';
    ...
    echo '<br />Processing Temp (TD, T), '.date("F j, Y, g:i a").'<br />';
    ...
    echo '<br />Processing Relative Humidity (RH), '.date("F j, Y, g:i a").'<br />';
...
//          echo $query.'<br />';
    ...
    echo '<br />Processing Fuel (FUELM, FUELT), '.date("F j, Y, g:i a").'<br />';
