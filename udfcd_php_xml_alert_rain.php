 ...
 */
  echo 'Beginning the PHP script that accesses and processes the XML data stream from the UDFCD rain XML file feeds containing the latest station rain data<br />';
  echo 'Processing began at '.date("F j, Y, g:i a").'<br />';
    /*
     *  define the XML source URL
     */
      $URL = 'http://www.udfcd.org/FWP/LDAD/alert_rain.xml';
      echo '  URL = '.$URL.'<br />';
    /*
     *  open the XML source
     */
      ...
      if( ! $xml = simplexml_load_string($xmlstring) ) {
        ...
        foreach( $xml as $response ) {
          ++$counter2;
          $query = "INSERT INTO hydromodels.rawdata_alert_rain(point_id,last_rpt,min_5,min_10,min_15,min_30,hour_1,hour_3,hour_6,hour_12,hour_24,day_7,day_21,shef_id,name,lat,long,qtime) VALUES (";
          $query .= $response->attributes()->id;
          $query .= ",'";
          $query .= $response->attributes()->last_rpt;
          $query .= "',";
          $query .= $response->attributes()->min_5;
          $query .= ",";
          $query .= $response->attributes()->min_10;
          $query .= ",";
          $query .= $response->attributes()->min_15;
          $query .= ",";
          $query .= $response->attributes()->min_30;
          $query .= ",";
          $query .= $response->attributes()->hour_1;
          $query .= ",";
          $query .= $response->attributes()->hour_3;
          $query .= ",";
          $query .= $response->attributes()->hour_6;
          $query .= ",";
          $query .= $response->attributes()->hour_12;
          $query .= ",";
          $query .= $response->attributes()->hour_24;
          $query .= ",";
          $query .= $response->attributes()->day_7;
          $query .= ",";
          $query .= $response->attributes()->day_21;
          $query .= ",'";
